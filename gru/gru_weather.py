...
# Sequence and prediction horizon (hourly, matching autoencoder)
...
# Build and train the GRU model for hourly prediction
model_file = os.path.join(dirName, "gru_precipitacao_model_hourly.h5")
if os.path.exists(model_file):
    ...
    if X_train is not None:
        print(f"Creating and training a new GRU model for hourly precipitation prediction...")
        ...
        gru_model = tf.keras.models.Sequential([
            tf.keras.layers.GRU(encoding_dim, activation='relu', input_shape=(input_dim, 1)),
            tf.keras.layers.RepeatVector(output_dim),
            tf.keras.layers.GRU(encoding_dim, activation='relu', return_sequences=True),
            tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(1))
        ])
        gru_model.compile(optimizer='adam', loss='mse')
        history = gru_model.fit(X_train, y_train, epochs=1000, batch_size=8, validation_split=0.1, verbose=0)
        ...
        print(f"Trained GRU model saved to '{model_file}'")
...
if gru_model is not None and len(scaled_data) > sequence_length_hours + prediction_horizon_hours:
    # Use the last available sequence to predict the next 168 hours
    last_sequence = scaled_data[-(sequence_length_hours + prediction_horizon_hours):-prediction_horizon_hours]
    # Ensure input has shape (1, timesteps, features)
    last_sequence = last_sequence.reshape(1, sequence_length_hours, 1)
    pred_out = gru_model.predict(last_sequence)
    # pred_out shape should be (1, prediction_horizon_hours, 1)
    pred_scaled = pred_out.reshape(-1)
    print(f"First 5 predictions (scaled): {pred_scaled[:5]}")
    # Check for NaNs before inverse transform
    if np.isnan(pred_scaled).any():
        print("Warning: model produced NaN predictions (scaled). Skipping inverse transform and metric calculation.")
        print(f"Raw model output (first 10): {pred_out.flatten()[:10]}")
        predictions = np.full(pred_scaled.shape, np.nan)
    else:
